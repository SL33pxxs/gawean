<div class="inline-block">
    @if (session('success')['jenis'] == 'success-create-kode' || session('success')['jenis'] == 'success-show-kode')
        ...
        <h4 class="bold font__size--18 text__18-1024 text__18-xs mb-3">{!!session('success')['jenis'] ==
            'success-create-kode' ?
        ...
        <h4 class="bold font__size--18 text__18-1024 text__18-xs mb-3">
            <a href="{{ asset('images/'.session('success')['image_name']) }}" target="_blank"
                class="btn btn-primary">PREVIEW IMAGE</a>
        </h4>
        @endif
        ...
        <h4 class="bold font__size--18 text__18-1024 text__18-xs mb-3">
            <div class="form-group">
                <textarea class="form-control" rows="3" id="resultText" readonly>{!! session('success')['url'] !!}</textarea>
            </div>
        </h4>
        <button class="btn btn-primary"
            id="copy-text"> SALIN TEXT </button>
        @endif


        @if (session('success')['jenis'] == 'success-create-kode')
    ...
    <h5 class="semi-bold font__size--14 text__14-1024 color__gray-1 mb-3"> IMAGE </h5>
    <h4 class="bold font__size--18 text__18-1024 text__18-xs mb-3">
        <img src="{{ url(Storage::url(session('success')['image_name'])) }}" alt="image" class="img-thumbnail"
            style="width: 200px; height: 200px;">

        <a href="{{  url(Storage::url(session('success')['image_name'])) }}" target="_blank"
            class="btn btn-primary">PREVIEW IMAGE</a>
    </h4>
    <button class="btn btn-sm btn__outlined--black btn__more shadow mt-3 semi-bold no__opacity text-uppercase"
        id="copy"> SALIN {{env('APP_NAME')}} </button>
    ...
    <h4 class="bold font__size--18 text__18-1024 text__18-xs mb-3">
        <div class="form-group">
            <textarea class="form-control" rows="3" readonly>{!! session('success')['url'] !!}</textarea>
        </div>
    </h4>
    <button class="btn btn-sm btn__outlined--black btn__more shadow mt-3 semi-bold no__opacity text-uppercase"
        id="copy"> SALIN {{env('APP_NAME')}} </button>
